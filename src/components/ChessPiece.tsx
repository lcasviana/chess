...
export const ChessPiece = ({ type, color, className = "" }: ChessPieceProps) => {
  const pieceColor = color === "w" ? "text-white" : "text-black";
  const pieceShadow = color === "w" ? "drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]" : "drop-shadow-[0_2px_2px_rgba(255,255,255,0.8)]";
  ...
  return <div class={`${pieceColor} ${pieceShadow} ${className} transition-all duration-300 ease-out`}>{renderPiece()}</div>;
};
